...
####################################### DISCLAIMER ########################################
|                        TEAM X DDOS 9WET X3 /  TEAM X MYGDO HAD 
###########################################################################################
...
while True:
    ...
    try:
        ...
        query = input("[*] Use Shodan API to search for affected BAL7A X TEAM servers? <Y/n>: ").lower()
        if query.startswith('y'):
            print('')
            print('[~] Checking Shodan.io API Key: %s' % SHODAN_API_KEY)
            results = api.search('product:"BAL7A X TEAM" port:11211')
